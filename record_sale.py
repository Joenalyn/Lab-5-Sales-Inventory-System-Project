...
from db_functions import get_all_products, record_sale


class RecordSaleWindow(QWidget):
    ...
    def build_ui(self):
        ...
        title = QLabel("Record Sale")
        title.setStyleSheet("font-size: 30px; font-weight: 800; color: #111; background: transparent; border: none;")
        subtitle = QLabel("Select a product, enter quantity sold, then save the sales transaction.")
        ...
        self.qty_input.setPlaceholderText("Enter quantity sold")
        ...
        """)
        save_btn.clicked.connect(self.handle_record_sale)
    ...
    def handle_record_sale(self):
        row = self.table.currentRow()
        if row < 0:
            QMessageBox.warning(self, "Record Sale", "Please select a product first.")
            return

        product_id = self.table.item(row, 0).text()
        quantity = self.qty_input.text().strip()

        success, message = record_sale(product_id, quantity)
        QMessageBox.information(self, "Record Sale", message)
